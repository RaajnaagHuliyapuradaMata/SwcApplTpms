...
static tProcIdBufType ProcIdBuf[TELEGRAM_QUEUE_SIZE] = {
   { cInvalidREid,cReIdProcBufTimeLimit },{ cInvalidREid,cReIdProcBufTimeLimit },
   { cInvalidREid,cReIdProcBufTimeLimit },{ cInvalidREid,cReIdProcBufTimeLimit },
   { cInvalidREid,cReIdProcBufTimeLimit },{ cInvalidREid,cReIdProcBufTimeLimit },
   { cInvalidREid,cReIdProcBufTimeLimit },{ cInvalidREid,cReIdProcBufTimeLimit },
   { cInvalidREid,cReIdProcBufTimeLimit },{ cInvalidREid,cReIdProcBufTimeLimit }
...
uint8 SetParkenWohnenFahrenDM( Rdci_CON_VEH_Type tPWF)
{
  ...
  if(tPWF.ST_CON_VEH != ucGetStatusConditionVehicleFZZ())
  {
    ...
    {
      if(tPWF.ST_CON_VEH == ST_CON_VEH_Standfunktionen_Kunde_nicht_im_FZG)
      {
        retVal = eParkenStandfunktion;
      }
      else if(tPWF.ST_CON_VEH == ST_CON_VEH_Wohnen)
      {
        retVal = eParkenWohnen;
      }
      else{}
    }
    else if(ucLastCond < ST_CON_VEH_Wohnen)
    {
      if(tPWF.ST_CON_VEH == ST_CON_VEH_Wohnen)
      {
        retVal = eParkenWohnen;
      }
      else if((tPWF.ST_CON_VEH == ST_CON_VEH_Parken_BN_iO) || (tPWF.ST_CON_VEH == ST_CON_VEH_Parken_BN_niO))
      {
        retVal = eStandFktParken;
      }
      else{}
    }
    else if(ucLastCond == ST_CON_VEH_Wohnen)
    {
      if(tPWF.ST_CON_VEH < ST_CON_VEH_Wohnen)
      {
        retVal = eWohnenParken;
      }
      else if(tPWF.ST_CON_VEH > ST_CON_VEH_Wohnen)
      {
        retVal = eWohnenFahren;
      }
      else
      {}
    }
...
void ExternalTemperatureTimerTickDM(void)
{
  ...
  if((ushSpeed != 0xffffU) && (ushSpeed >= 20))
  {
    if(ucExtTemperatureTimer < 30)
    {
...
void SetGNSSErrorAltitudeMetersDM(Rte_Instance self, Rdci_GNSSErrorAltitudeMeters_Type GNSSErrorAltitudeMeters, uint8 ucError)
{
  ...
  if( ucError == cNoError)
  {
    ...
    else
    {
      ...
      if( (ucState & cAGS_GpsAltSigValid) == cAGS_GpsAltSigValid)
      {
...
void SetSysTimeDM( uint32 ulSysTime)
{
  ulRelTimeSinceSysStartDM = ulSysTime;
}

uint32 ulGetSysTimeDM(void)
{
  return ulRelTimeSinceSysStartDM;
}

void SetStbMBSyncStateDM(boolean bSync)
{
  bStbMBSyncState = bSync;
}

boolean bGetStbMBSyncStateDM(void)
{
  return bStbMBSyncState;
}

uint32 ulGetStopTimeDM(void)
{
  return ulStopTime;
}

void ClearStopTimeDM(void)
{
  ulStopTime = 0;
}
...
uint8 ucGetRdcEventCounterDM(void)
{
  return (ucRxEventCounter);
}
...
uint8 ucSetRdcDataDM(const ImpTypeRecCddRdcData* data, uint8 ucType)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if( ucRdcQueueReadPos == 0xFFU)
      {
        queueFillStatus = ucRdcQueueWritePos;
      }
      else
      {
        if(ucRdcQueueWritePos > ucRdcQueueReadPos)
        {
          queueFillStatus = ucRdcQueueWritePos - ucRdcQueueReadPos;
        }
        else
        {
          queueFillStatus = TELEGRAM_QUEUE_SIZE - (ucRdcQueueReadPos - ucRdcQueueWritePos);
        }
      }
    }
  }
...
uint8 ucGetRdcDataDM(ImpTypeRecCddRdcData* data, uint8* ucType)
{
  ...
  if(ucRdcQueueWritePos != 0xFFU)
  {
    ...
    if(ucRdcQueueReadPos == 0xFFU)
    {
      if(RdcDataQueue[0].ucWaitingTime <= cTEL_WAIT_TIME)
      {
    ...
    {
      if( ucRdcQueueReadPos == ucRdcQueueWritePos)
      {

        bReadQueueData = FALSE;
      }
      else if(RdcDataQueue[(ucRdcQueueReadPos + 1) % TELEGRAM_QUEUE_SIZE].ucWaitingTime <= cTEL_WAIT_TIME)
      {
...
uint8 ucCalcBatteryLevelDM(const ImpTypeRecCddRdcData* rdcData)
{
  ...
  switch (rdcData->PCKG_ID)
  {
    ...
    ucBatStatus *= 10;
    if((ucBatStatus == 0) || (ucBatStatus > 110))
    {
      ucBatStatus = cBATTERY_STATUS_UNKNOWN;
    }
    else if(ucBatStatus > 100)
    {
      ucBatStatus = 100;
    }
    else
    ...
    if(ucBatStatus <= 120)
    {
      ucBatStatus = (uint8)(((uint16)ucBatStatus * 100) / 120);
    }
    else
    {
      ucBatStatus = cBATTERY_STATUS_UNKNOWN;
...
void SetLoadStateDM(const uint8 LoadState)
{
	...
	{
		MaxLoadStateVal = OP_TAR_P_LOCO_TPCT_Signal_ungueltig;
	}
	else
	{
		MaxLoadStateVal = OP_TAR_P_LOCO_TPCT_ECO_Solldruck;
	}
...
uint8 GetLoadStateDM(void)
{
	return (ucLoadStateOut);
}
...
uint8 GetSeasonDM(void)
{
	return (ucSeasonOut);
}

void SetWinterTyreIndexDM(const uint8 Index)
{
	ucWITRIndex = Index;
}

uint8 GetWinterTyreIndexDM(void)
{
	return (ucWITRIndex);
}

void SetSummerTyreIndexDM(const uint8 Index)
{
	ucSUTRIndex = Index;
}

uint8 GetSummerTyreIndexDM(void)
{
	return (ucSUTRIndex);
}

void SetIDRMessageCenterDM(const uint8 Idr)
{
  ucIDRMSGC = Idr;
}

uint8 GetIDRMessageCenterDM(void)
{
	return (ucIDRMSGC);
}

void SetStatusManSelectionDM(const uint8 MaSel)
{
  ucSTMANSLCTN = MaSel;
}

uint8 GetStatusManSelectionDM(void)
{
	return (ucSTMANSLCTN);
}

void SetLengthOfTyreListDM(const uint8 Length)
{
  ucLengthOfTyreList = Length;
}

uint8 GetLengthOfTyreListDM(void)
{
	return (ucLengthOfTyreList);
}

void SetSelectedTyreIndexDM(const uint8 Index)
{
	if((Index < GetLengthOfTyreListDM()) || (Index == OP_SLCTN_TYR_AVLB_AndererReifen))
	{
		ucSLCTNTRIndex = Index;
	}
}

uint8 GetSelectedTyreIndexDM(void)
{
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
              ...
              if((pConvRidData[j] & CheckMask) == CSEASON_SUMMER)
              {

                if((MemValue & CheckMask) != CSEASON_SUMMER)
                {
...
static void CompareSpeedIxToErfsDM(const uint8* pConvRidData, uint8* pIxArray)
{
  ...
  for (i=0; i<TYRE_LIST_MAX_ELEMENTS ;i++)
  {
    ...
    if(pIxArray[i] != 0xFF)
    {
      ...
      if((GetTyreListMember (i, LOAD_SEAS_IX) & 0x03) == CSEASON_SUMMER)
      {
...
static boolean ComparePressValOfMatchingErfsEntriesDM(const uint8* pIxArray)
{
  ...
  for (i=0; i<TYRE_LIST_MAX_ELEMENTS ;i++)
  {
    if(pIxArray[i] != 0xFF)
    {
      ...
      {
        if(uc1stPressByte[0] != GetTyreListMember (i, PRESS_BYTE_1_IX))
        {
          bComp = FALSE;
        }
        if(uc1stPressByte[1] != GetTyreListMember (i, PRESS_BYTE_2_IX))
        {
          bComp = FALSE;
        }
        if(uc1stPressByte[2] != GetTyreListMember (i, PRESS_BYTE_3_IX))
        {
           bComp = FALSE;
        }
      }
...
uint8 GetDefaultLoadCondDM(void)
{
	...
	if(bGetCRdciEcoRcpEnableCD() == TRUE)
	{
		if(bGetCRdciDefaultLoadCondCD() == TRUE)
		{
			DefLoadCon = REQ_ECO_LOAD;
		}
		else
		{
			DefLoadCon = REQ_PART_LOAD;
		}
	}
	else
	{
...
uint8  ucGetRePckgIdDM( uint8 ucSlotNo)
{
  return ucRePckgIdDM[ucSlotNo];
}

uint32 ulGetReIdDM( uint8 ucSlotNo)
{
  return ulReIdDM[ucSlotNo];
}

uint8  ucGetReSuppIdDM( uint8 ucSlotNo)
{
  return ucReSuppIdDM[ucSlotNo];
}

PhySensorTyrePresType  ucGetRePressureRelDM( uint8 ucSlotNo)
{
  return ucRePressureRelDM[ucSlotNo];
}

PhySensorTyreTempType  scGetReTemperatureCentDM( uint8 ucSlotNo)
{
  return scReTemperatureCentDM[ucSlotNo];
}

uint8 ucGetRePalLookBackTimeDM( uint8 ucSlotNo)
{
  return ucRePalLookBackTimeDM[ucSlotNo];
}

uint16 ushGetRePalStatusDM( uint8 ucSlotNo)
{
  return ushRePalStatusDM[ucSlotNo];
}

uint8 ucGetTelRssiLevelDM( uint8 ucSlotNo)
{
  return ucTelRssiLevelDM[ucSlotNo];
}

uint8 ucGetReBatteryLevelDM( uint8 ucSlotNo)
{
  return ucReBatteryLevel[ucSlotNo];
}

void PutRePckgIdDM( uint8 ucSlotNo, uint8 ucPckgId)
{
  if(ucSlotNo < cAnzRad)
  {
    ucRePckgIdDM[ucSlotNo] = ucPckgId;
  }
}

void PutReIdDM( uint8 ucSlotNo, uint32 ulId)
{
  if(ucSlotNo < cAnzRad)
  {
    ulReIdDM[ucSlotNo] = ulId;
  }
}

void PutReSuppIdDM( uint8 ucSlotNo, uint8 ucSuppId)
{
  if(ucSlotNo < cAnzRad)
  {
    ucReSuppIdDM[ucSlotNo] = ucSuppId;
  }
}

void PutRePressureRelDM( uint8 ucSlotNo, PhySensorTyrePresType ucPressure)
{
  if(ucSlotNo < cAnzRad)
  {
    ucRePressureRelDM[ucSlotNo] = ucPressure;
  }
}

void PutReTemperatureCentDM( uint8 ucSlotNo, PhySensorTyreTempType scTemperature)
{
  if(ucSlotNo < cAnzRad)
  {
    scReTemperatureCentDM[ucSlotNo] = scTemperature;
  }
}

void PutRePalLookBackTimeDM( uint8 ucSlotNo, uint8 ucPalLookBackTime)
{
  if(ucSlotNo < cAnzRad)
  {
    ucRePalLookBackTimeDM[ucSlotNo] = ucPalLookBackTime;
  }
}

void PutRePalStatusDM( uint8 ucSlotNo, uint16 ushPalStatus)
{
  if(ucSlotNo < cAnzRad)
  {
    ushRePalStatusDM[ucSlotNo] = ushPalStatus;
  }
}

void PutTelRssiLevelDM( uint8 ucSlotNo, uint8 ucRssi)
{
  if(ucSlotNo < cAnzRad)
  {
    ucTelRssiLevelDM[ucSlotNo] = ucRssi;
  }
}

void PutReBatteryLevelDM( uint8 ucSlotNo, uint8 ucBatLevel)
{
  if(ucSlotNo < cAnzRad)
  {
    ucReBatteryLevel[ucSlotNo] = ucBatLevel;
  }
}
...
static uint8 ucConvAkAbsPressureInRelDM( uint8 ucRawPres, PhySensorTyrePresType* pucPhyPres, uint8 ucPamb)
{
  ...
  switch( ucRawPres)
  {
    ...
    default:
      ...
      {
        if( (ushPsensorAbs - (uint16) ucPamb) > (uint16) 252)
        {

          *pucPhyPres = 252;
          ucError = cPresSensorOverflow;
        }else{
          *pucPhyPres = (PhySensorTyrePresType) (ushPsensorAbs - ucPamb);

          ucError = cPresSensorOk;
        }
      }else{
...
PhySensorTyrePresType ucGetValidTyrePressureRelAtWheelPosDM( uint8 ucWheelPos)
{
  ...
  if( ucHisPos < cAnzRad)
  {
    ucRet = ucGetRePressureRelDM( ucHisPos);
  }else{
...
sint8 scGetValidTyreTemperatureAtWheelPosDM( uint8 ucRadPos)
{
  ...
  if( ucHisPos < cAnzRad)
  {
    scRet = scGetReTemperatureCentDM( ucHisPos);
  }else{
    scRet = cInvalidREtemperature;
  }
...
void CheckTyreChangedDM(Rte_Instance self)
{
  ...
  if(GETStManSelectionEE(self) == ST_MAN_SLCTN_Manuelle_Reifenauswahl)
  {
    ...
    for (i=0;i<cAnzRad;i++)
    {
      if((IdChangeBits & (uint8)(1<<i)) == ((uint8)(1<<i)))
      {
        BitCt+=0x01;
      }
      if((WpChangeBits & (uint8)(1<<i)) == ((uint8)(1<<i)))
      {
        BitCt+=0x10;
      }
    }
...
void DegradeToManSelectionDM (Rte_Instance self, boolean bOtherTyre)
{
  ...
  if(GETStManSelectionEE(self) == OP_IDR_SLCTN_Automatische_Auswahl)
  {
    ...
    if(bOtherTyre == TRUE)
    {
      ...
      {
        SetWinterTyreIndexDM(OP_SLCTN_TYR_AVLB_AndererReifen);
      }
    }
    else
    {
...
static uint8 ucCalcSignalToNoiseRatio(uint8 ucTelRssi, uint8 ucBackgroundNoise)
{
  ...
  if(ucBackgroundNoise == 0xff)
  {
    ucRet = 0xff;
  }
  else if(ucBackgroundNoise > ucTelRssi)
  {
    ucRet = 0xff;
  }
  else
  {
    ucRet = ucTelRssi - ucBackgroundNoise;
  }
  return ucRet;
}
...
uint8 GetStWheelTypeChangedDetStateDM(void)
{
  ...
  if(0 != (ushBZ & (cLOC_INTERRUPTED | cLOC_NOT_POSSIBLE | cZO_TIMEOUT)))
  {
    ucRetState = StatusWheelTypeChangeDetection_Algorithmus_Raderkennung_aZ6Y7PL;
  }
  else if(cZO_FINISH == (ushBZ & cZO_FINISH))
  {
    ucRetState = StatusWheelTypeChangeDetection_Positionsbestimmung_abgesc2IYJF8;
  }
  else if(cER_FINISH == (ushBZ & cER_FINISH))
  {
    ucRetState = StatusWheelTypeChangeDetection_Radidentifikation_abgeschlossen;
  }
  else
  {
    ucRetState = StatusWheelTypeChangeDetection_Algorithmus_Raderkennung_laeuft;
  }
  return ucRetState;
}
...
void DeleteReIdLatelyProcBufDM(void)
{
  ...
  for (i=0;i<TELEGRAM_QUEUE_SIZE;i++)
  {
    ...
    if(ProcIdBuf[i].ucTimeCt != cReIdProcBufTimeLimit)
    {
  ...
  if(ucReIdBufTimeCt == (cReIdProcBufTimeLimit - 1))
  {
    ucReIdBufTimeCt = 0x00;
  }
  else
  {
    ucReIdBufTimeCt++;
